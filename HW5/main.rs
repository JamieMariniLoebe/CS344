...
use std::thread; // Adding for thread usage
...
fn main() {
    ...
    // Clone xs so as to use it with threads
    let _xs2 = xs.clone();

    // Creating the 2 threads, move in data
    let _thread1 = thread::spawn(move || map_data(&xs[0]));
    let _thread2 = thread::spawn(move || map_data(&_xs2[1]));
  
    // Join threads and end them
    let _jthread1 = _thread1.join().unwrap();
    let _jthread2 = _thread2.join().unwrap();
  
    // Push each result to intermediate sums
    intermediate_sums.push(_jthread1);
    intermediate_sums.push(_jthread2);
    ...
    // Creating a vector of partitions using # of partitions passed in
    let vec_partitions = partition_data(num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&vec_partitions);

    // Creating vector to hold sums from each thread
    ...
    // Loop for each paritition
    for t in 0..num_partitions {

        // Create clone for each vector processed by each thread
        let vec_clone = vec_partitions.clone();

        // Create new thread and process the vector at [t]
        let new_thread = thread::spawn(move||map_data(&vec_clone[t]));
        let final_thread = new_thread.join().unwrap();

        // 4. Collects the intermediate sums from all the threads

        // Push map_data result to interemediate_sums_2 vector
        intermediate_sums_2.push(final_thread);
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // Allow for adding to partitions
    ...
    for i in 0..num_partitions {
        ...
        // Push each val into a partition
        for j in start..end{
            vect.push(v[j])
        }
        xs.push(vect);

        // If size is uneven, add val to vector under mod is 0
        if mod_val > 0 {
            xs[i].push(v[end]);
